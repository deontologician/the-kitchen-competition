...
- **`KitchenScene`** — Dual-mode. **Prep mode** (`kitchen_prep` phase): 30s countdown, auto-transitions to RestaurantScene. **Cooking mode** (`service` phase, `cooking` sub-phase): shows order details + recipe list. Uses `kitchenVM` for recipe craftability (filtered by `disabledDishes`). Service timer keeps ticking. Esc → PauseScene. **Recipe list scrolling:** all enabled recipes are shown (no 7-item cap); rows are rendered into a `scrollContainer` (Container at origin) with a geometry mask clipping to `recipeRegion`. Mouse-wheel scrolls when hovering the panel; ▲/▼ arrow Text buttons scroll by one `RECIPE_ROW_H` step. `scrollOffset` / `maxScrollOffset` instance vars track position. Wheel listener registered once in `create()`, cleaned up on `shutdown`. `renderRecipeList()` resets scroll to 0 and destroys/recreates the container + mask + arrows on each call.
