...
- **`menu.ts`** — Per-restaurant-type menu definitions with progressive dish unlocking. Types: `MenuItem` (dishId + sellPrice), `MenuDef` (restaurantType + items). 5 dishes per type with sell prices ($5-12), ordered by unlock sequence (index 0 = starter dish). `STARTER_DISH_COUNT = 1`. Exports: `menuFor`, `dishIdsFor`, `groceryItemsFor`, `availableRecipesFor`, `pickRandomDish` (all operate on full menu); `unlockedMenuFor(type, count)`, `unlockedDishIdsFor`, `unlockedGroceryItemsFor`, `unlockedRecipesFor` (restrict to first `count` dishes); `shouldUnlockNextDish(customersLost, coins, currentUnlocked, maxDishes?)` — returns new unlock count (+1 if 0 lost, coins > 0, room to unlock; unchanged otherwise).
...
- **`format.ts`** — Shared formatting utilities. Types: `TimerColor` (`"green" | "yellow" | "red"`), `FreshnessLevel` (`"fresh" | "warning" | "critical"`), `PatienceLevel` (`"ok" | "warning" | "critical"`). Exports: `formatTimeRemaining(ms)` (returns `"M:SS"`), `truncateName(name, maxLen)`, `timerColor(fraction)`, `freshnessLevel(fraction)`, `patienceLevel(fraction)`. Threshold: >50% = green/fresh/ok, >25% = yellow/warning, ≤25% = red/critical.
