...
- **`RestaurantScene`** — Service phase hub. Loads themed background + table sprites (2x3 grid at `TABLE_POSITIONS`). Timer bar shows "DAY X - SERVICE M:SS". Uses `difficultyForDay(cycle.day)` for spawn timing, patience ranges, and max customers per day. Table tints: white (empty), green/yellow/red (patience level via `patienceColor()`), blue (active customer). Order bubbles (32px dish sprites) + patience bars rendered above occupied tables. Auto-calls `beginTakingOrder` when waiting + queue non-empty. "Serve Now" button if dish in inventory (skips kitchen trip via instant `beginCooking` → `finishCooking` → `finishServing`), else "Cook Order" → KitchenScene. Customer-left notifications with fade-out tween. Day-end overlay: customers served, customers lost (red, if > 0), earnings, total wallet, "Next Day" button. Esc pauses. Helper methods: `getPriceForDish`, `removeFromInventory`.
