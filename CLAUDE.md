...
- `algebraic-properties.test.ts` — Cross-module property-based tests for algebraic invariants (wallet monoid, inventory consistency, removeExpired idempotence, timer/difficulty monotonicity, menu prefix, leaderboard max, classify3 boundaries, spend/afford consistency).
...
- **`restaurant-type.ts`** — Core `RestaurantType` definition (`"sushi" | "bbq" | "burger"`) and `restaurantDisplayName`. Extracted from `save-slots.ts` since 9+ modules depend on this type. Canonical import source for domain modules.
- **`save-slots.ts`** — Multi-slot save system (v2). Re-exports `RestaurantType` and `restaurantDisplayName` from `restaurant-type.ts` for backward compatibility. Types: `SaveSlot` (id, restaurantType, day, coins, scene, lastSaved, unlockedDishes), `SaveStore` (version 2 + slots array). Pure functions: `createSaveSlot` (unlockedDishes defaults to 5 for backward compat), `createSaveStore`, `addSlot`, `updateSlot`, `removeSlot`, `findSlot`, `findMostRecent`, `serializeStore`, `deserializeStore` (handles missing unlockedDishes in persisted data), `loadStore` (v2 parse → v1 migration → empty), `sceneDisplayName`, `formatSlotSummary`. All timestamps/IDs are parameters (no side effects).
...
- **`recipes.ts`** — Recipe step definitions and dependency chain resolution. Types: `RecipeMethod` (`"prep" | "cook" | "assemble"`), `RecipeInput` (itemId + quantity), `RecipeStep` (id, name, inputs, output, method, timeMs), `RecipeNode` (step + children tree). 46 recipe steps: prep/cook for intermediates, assemble for dishes (timeMs 0), miso soup is the only cook-method dish (5s). Core abstraction: `foldRecipeTree(node, leaf, merge)` — generic catamorphism over recipe trees with dedup-by-seen-set; `leaf` handles already-visited nodes, `merge` combines step with child results. Derived: `flattenRecipeChain` (topological order, leaves first), `totalRawIngredients` (aggregates raw costs), `totalRecipeTime`. Also exports: `findRecipe`, `recipesForOutput`, `allRecipes`, `resolveRecipeChain` (builds full dependency tree).
...
- **`menu.ts`** — Per-restaurant-type menu definitions with progressive dish unlocking. Types: `MenuItem` (dishId + sellPrice), `MenuDef` (restaurantType + items). 5 dishes per type with sell prices ($5-12), ordered by unlock sequence (index 0 = starter dish). `STARTER_DISH_COUNT = 1`. Core implementations are the `unlocked*` variants; full-menu functions delegate with `count=Infinity`: `menuFor` → `unlockedMenuFor`, `dishIdsFor` → `unlockedDishIdsFor`, `groceryItemsFor` → `unlockedGroceryItemsFor`, `availableRecipesFor` → `unlockedRecipesFor`. Also exports `pickRandomDish`, `shouldUnlockNextDish(customersLost, coins, currentUnlocked, maxDishes?)` — returns new unlock count (+1 if 0 lost, coins > 0, room to unlock; unchanged otherwise).
...
### View Model Layer (`src/domain/view/`)

Pure functions that bridge domain state to scene-renderable data. Zero Phaser imports. Scenes call these to compute display state, then render the result.

- **`format.ts`** — Shared formatting utilities. Types: `TimerColor` (`"green" | "yellow" | "red"`), `FreshnessLevel` (`"fresh" | "warning" | "critical"`), `PatienceLevel` (`"ok" | "warning" | "critical"`). Internal `classify3<T>(fraction, high, mid, low)` generic implements the shared three-bucket classifier (>50% / >25% / ≤25%). Exports: `formatTimeRemaining(ms)` (returns `"M:SS"`), `truncateName(name, maxLen)`, `timerColor(fraction)`, `freshnessLevel(fraction)`, `patienceLevel(fraction)` — all three delegate to `classify3`.
- **`timer-vm.ts`** — Timer bar display state. `TimerBarVM` (fraction, label, color). `timerBarVM(phase, day)` returns `undefined` for `day_end`. Label format: `"DAY {day} - {PHASE_NAME} {M:SS}"` where phase names are SHOPPING/PREPPING/SERVICE.
- **`inventory-vm.ts`** — Grouped inventory display. `InventoryItemVM` (itemId, displayName, count, freshness). `InventoryVM` (dishes, prepped, hasDivider, isEmpty). `inventoryVM(inventory, now)` groups by category, truncates names to 12 chars, computes freshness levels. Only includes dish and prepped categories.
- **`grocery-vm.ts`** — Grocery scene item list. `GroceryItemVM` (itemId, name, displayName, cost, count, canAfford, spriteKey). `GroceryVM` (items). `groceryVM(wallet, inventory, restaurantType, unlockedCount)` computes per-item affordability and inventory counts with names truncated to 10 chars.
- **`kitchen-vm.ts`** — Recipe list and cooking progress. `ActiveRecipe` (step + startedAt), `RecipeInputVM` (itemId, name, have, need, isShort), `RecipeVM` (stepId, outputName, outputSpriteKey, inputs, timeSeconds, canMake, method), `ActiveRecipeVM` (outputName, fraction). `KitchenVM` (recipes, activeRecipe). `kitchenVM(inventory, restaurantType, unlockedCount, activeRecipe, now)`.
- **`day-end-vm.ts`** — Day end summary and dish unlock. `DishUnlockVM` (dishId, dishName, dishSpriteKey, newUnlockedCount). `DayEndVM` (day, customersServed, customersLost, earnings, newTotalCoins, dishUnlock). `dayEndVM(phase, day, wallet, restaurantType, currentUnlocked)` computes earnings total and unlock decision.
- **`restaurant-vm.ts`** — Table status and action prompts. `TableTint` (`"neutral" | "ok" | "warning" | "critical" | "active"`), `TableVM` (tableId, occupied, customerId, dishSpriteKey, patienceFraction, tint, showPatienceBar). `ActionPrompt` (discriminated: `waiting` | `taking_order` | `cooking` | `serving`). `RestaurantVM` (tables, actionPrompt). `restaurantVM(phase, inventory, restaurantType, unlockedCount)`.
...
- **`GroceryScene`** — Grocery store. Loads themed background via `backgroundKey(type, "grocery")`. Uses `groceryVM` for item grid (affordability, counts, truncated names) and `timerBarVM` for timer label/color. Calls `recordSceneEntry` on create. Renders panel overlay + pixel-art title + coin HUD (top-right). Timer bar at top shows "SHOPPING" countdown (30s). `update()` ticks timer via `tickTimer`, redraws bar. When expired, auto-transitions to KitchenScene via `advanceToKitchenPrep`. Esc key pauses and launches PauseScene.
- **`KitchenScene`** — Dual-mode kitchen. Loads themed background via `backgroundKey(type, "kitchen")`. Uses `kitchenVM` for recipe list display (craftability, input have/need, active progress) and `timerBarVM` for timer labels. Renders panel overlay. **Prep mode** (`kitchen_prep` phase): timer bar with "PREPPING" countdown (30s), auto-transitions to RestaurantScene via `advanceToService`. **Cooking mode** (`service` phase, `cooking` sub-phase): shows order details, recipe list for crafting. Service timer keeps ticking during cooking; if it expires, transitions to day end. Esc key pauses and launches PauseScene.
- **`RestaurantScene`** — Service phase hub. Loads themed background + table sprites (2x3 grid at `TABLE_POSITIONS`). Uses `timerBarVM` for timer display, `restaurantVM` for action prompts (waiting/taking_order/cooking/serving with dish info, sell price, hasDish), and `dayEndVM` for day-end summary and dish unlock logic. Uses `difficultyForDay(cycle.day)` for spawn timing, patience ranges, and max customers per day. Delegates to helpers: `renderTableOverlays`, `renderInventorySidebar`, `animateServe`/`animateCustomerLeft`/`animateArrival`, `showNotification`. Auto-calls `beginTakingOrder` when waiting + queue non-empty. "Serve Now" button if dish in inventory (skips kitchen trip via `doServe`), else "Cook Order" → KitchenScene. Day-end overlay: customers served, customers lost, earnings, total wallet, dish unlock banner. "Next Day" button. Esc pauses.
...
- **`timerBar.ts`** — Timer bar rendering helper. `renderTimerBar(scene, x, y, width, height, fraction, options?)` returns `TimerBarResult` (graphics + label) for proper cleanup — eliminates fragile `children.list` indexing. `TIMER_COLOR_HEX` maps `TimerColor` to hex values. Re-exports `formatTimeRemaining` from `domain/view/format.ts`.
